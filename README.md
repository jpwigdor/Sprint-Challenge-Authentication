...
1. What is the purpose of using _sessions_?
   Sessions are used to persist state between page requests. This commonly involves cookies.

2. What does bcrypt do to help us store passwords in a secure manner.
   bcrypt preforms Hashing which is a 1-way algorithm.

3. What does bcrypt do to slow down attackers?

- a few things! It can add salt which adds random string characters to the password before hashing it.
- It can Hash numerous times on top of eachother and it's suggested to has as high as possible until it slows down the User Experience to a noticable level.

4. What are the three parts of the JSON Web Token?

- Header = identifies the algorithm used to generate the token and the type.
- Payload = Contains the data passed into the token. You want as little information in here as possible as this can be decoded easily by something like the www.jwt.io Debugger.
- Signiture = This validates the token and makes sure that the client didn't modify the token's role. (a client changing it to pretend to be an admin) as this uses a secret string for it's encoding.
